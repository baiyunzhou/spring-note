...
/**
用于在注解元素上查找注解、元注解和可重复注解的通用实用工具方法。

AnnotatedElementUtils定义了Spring元注解编程模型的公共API，支持注解属性覆盖。如果不需要对注解属性重写的支持，可以考虑使用AnnotationUtils。

注意，该类的特性不是JDK的自省工具本身提供的。

注解属性重写

getMergedAnnotationAttributes()、getMergedAnnotation()、getAllMergedAnnotations()、getMergedRepeatableAnnotations()、findMergedAnnotationAttributes()、findMergedAnnotation()、findAllMergedAnnotations()和findMergedRepeatableAnnotations()方法的所有变体都提供了对带有属性重写的元注解的支持。

Find vs. Get 语义

该类中方法使用的搜索算法遵循find或get语义。有关使用哪种搜索算法的详细信息，请参阅每个方法的javadoc。

Get语义-仅限于搜索注解元素(即在本地声明的或继承的)或在注解元素之上的注解层次结构中声明的注解。

Find语义-更加详尽，提供了get语义以及对以下内容的支持:
- 如果带注解的元素是类，则在接口上进行搜索
- 如果带注解的元素是类，则在超类上进行搜索
- 如果带注解的元素是方法，则解析桥接方法
- 如果带注解的元素是方法，则搜索接口中的方法
- 如果带注解的元素是方法，则在超类中搜索方法

@Inherited支持

遵循get语义的方法将遵守Java的@Inherited注解的契约，但是本地声明的注解(包括自定义组合的注解)将优先于继承的注解。相反，遵循find语义的方法将完全忽略@Inherited的存在，因为find搜索算法手工遍历类型和方法层次结构，因此隐式地支持注解继承，而不需要@Inherited。
**/  
public class AnnotatedElementUtils {
	...
	/**
	用于在搜索期间处理注解的回调接口。
	根据用例，处理器可以选择处理单个目标注释、多个目标注释或当前执行搜索发现的所有注释。在此上下文中，术语“目标”指的是匹配的注释(即，在搜索过程中找到的特定注释类型)。
	从process方法返回一个非空值，指示搜索算法停止进一步搜索;然而，从process方法返回null指示搜索算法继续搜索其他注释。这个规则的一个例外适用于聚合结果的处理器。如果聚合处理器返回一个非空值，该值将被添加到聚合结果列表中，搜索算法将继续。
	当搜索算法从返回非空值的流程调用返回到作为搜索算法起点的AnnotatedElement时，处理器可以选择性地对流程方法的结果进行后处理。
	 **/
